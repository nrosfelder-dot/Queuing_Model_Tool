...
# ==========================================
class Station:
    def __init__(self, env, name, capacity, mean_service_time, mandatory_rest_time):
        self.env = env
        self.name = name
        self.capacity = capacity
        self.server = simpy.Resource(env, capacity=capacity)
        self.mean_service_time = mean_service_time
        self.mandatory_rest_time = mandatory_rest_time

        # Metric Tracking
        self.mandatory_rest_times = []  # Time truck HAS to rest
        self.queue_times = []           # Time truck waits AFTER resting because machine is busy
        self.process_times = []         # Time inside the machine
        self.total_busy_time = 0.0

        # Physical Inventory Tracking
        self.trucks_in_storage = 0
        self.storage_inventory_log = []
        self.env.process(self.monitor_storage())

    def monitor_storage(self):
        """Logs the physical number of trucks sitting in the storage area every minute."""
        while True:
            self.storage_inventory_log.append(self.trucks_in_storage)
            yield self.env.timeout(1.0)

    def process_entity(self, entity_name):
        arrival_time = self.env.now
        
        # 1. TRUCK ENTERS STORAGE
        self.trucks_in_storage += 1
        
        # 2. MANDATORY REST/CHILL TIME
        yield self.env.timeout(self.mandatory_rest_time)
        end_rest_time = self.env.now
        self.mandatory_rest_times.append(end_rest_time - arrival_time)

        # 3. QUEUE FOR MACHINE (Wait until a server opens up)
        with self.server.request() as request:
            yield request 
            
            # 4. TRUCK ENTERS MACHINE (Leaves storage)
            start_service_time = self.env.now
            self.trucks_in_storage -= 1 
            
            queue_time = start_service_time - end_rest_time
    ...
    def get_metrics(self, total_sim_time):
        avg_rest = statistics.mean(self.mandatory_rest_times) if self.mandatory_rest_times else 0
        ...
        avg_trucks_in_storage = statistics.mean(self.storage_inventory_log) if self.storage_inventory_log else 0
        max_trucks_in_storage = max(self.storage_inventory_log) if self.storage_inventory_log else 0
        
        utilization = self.total_busy_time / (self.capacity * total_sim_time) if total_sim_time > 0 else 0
        
        # Total time spent in the physical staging area (Resting + Waiting for machine)
        total_time_in_storage = avg_rest + avg_queue
...
def entity_generator(env, arrival_mean, stations):
    ...
    while True:
        ...
        entity_count += 1
        env.process(route_through_line(env, f"Truck-{entity_count}", stations))
...
def run_tandem_simulation(sim_time, arrival_mean, station_configs):
    ...
    for config in station_configs:
        stations_list.append(
            Station(env, config["name"], config["capacity"], config["service_time"], config["rest_time"])
        )
...
st.write("Configure your production flow below. The model tracks both mandatory resting time and bottleneck waiting time in your storage areas.")
...
default_rest_times = [0.0, 15.0, 0.0, 120.0, 0.0, 0.0] # E.g., chill for 120 mins before cutting
...
for i in range(num_stations):
    ...
    d_rest = default_rest_times[i] if i < len(default_rest_times) else 0.0
    ...
    with st.container():
        ...
        with c3:
            rest_time = st.number_input("Mandatory Storage Rest (min)", min_value=0.0, value=d_rest, step=5.0, key=f"rest_{i}")
        with c4:
            svc_time = st.number_input("Machine Cycle Time (min)", min_value=0.05, value=d_time, step=0.5, key=f"time_{i}")
    ...
    with st.spinner('Simulating processing line dynamics...'):
        ...
        if not overutilized.empty:
            for _, row in overutilized.iterrows():
                st.error(f"⚠️ **{row['Station / Machine']}** is completely bottlenecked. The upstream storage area will overflow infinitely because the machine cannot keep up with the trucks coming in.")
